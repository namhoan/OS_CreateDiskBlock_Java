...
public class MyFileSystem implements FileSystem {
    ...
    private DirectBlock getDirectBlock(int fd, MODE mode) {//FIXME!!!
        ...
        // each disk can save #units of block index
        // assume each block index entry takes 4 bytes.
        int entrySize = 4;
        int unit = Disk.BLOCK_SIZE / entrySize;

        // check if it is in the single indirect block
        if (blockNum < 10 + unit) {
        	int index = 10;
        	...
        	if (fresh) {
        		...
        		if ((inode.ptr[index] = createDiskBlock()) == 0) {
        			// disk is full
        			return null;
        		}
        	}
        	// get the block number
        	int inblock = blockNum - 10;
        	int x = inblock / 1;
        	int blockNum1 = getBlockEntry(inode.ptr[index], entrySize, x);
        	fresh = blockNum1 == 0;
        	if (fresh) {
        		if(mode == MODE.r)
                    return DirectBlock.hole;
        		// allocate a free block to the new block
        		if ((blockNum1 = createDiskBlock(inode.ptr[index], entrySize, x)) == 0) {
        			// disk is full
        			return null;
        		}
        	}
        	// return the direct data block
        	return new DirectBlock(disk, blockNum1, blockOff, fresh);
        }
        
        // check if it is in the double indirect block
        if (blockNum < 10 + unit + unit * unit) {
        	int index = 11;
        	boolean fresh = inode.ptr[index] == 0; // no double indirect block yet
        	if (fresh) {
        		if(mode == MODE.r)
                    return DirectBlock.hole;
        		// otherwise, create the double indirect block
        		if ((inode.ptr[index] = createDiskBlock()) == 0) {
        			// disk is full
        			return null;
        		}
        	...
        	// now check the second indirect table
        	int blockNum2 = getBlockEntry(blockNum1, entrySize, y);
        	fresh = blockNum2 == 0;
        	if (fresh) {
        		if(mode == MODE.r)
                    return DirectBlock.hole;
        		// allocate a free block to the new block
        		if ((blockNum2 = createDiskBlock(blockNum1, entrySize, y)) == 0) {
        			// disk is full
        			return null;
        		}
        	}
        	// return the direct data block
        	return new DirectBlock(disk, blockNum2, blockOff, fresh);
        }
        
        // check if it is in the triple indirect block
        if (blockNum < 10 + unit + unit * unit + unit * unit * unit) {
        	int index = 12;
        	boolean fresh = inode.ptr[index] == 0; // no triple indirect block yet
        	if (fresh) {
        		if(mode == MODE.r)
                    return DirectBlock.hole;
        		// otherwise, create the double indirect block
        	...
        	int inblock = (blockNum - 10 - unit - unit * unit);
        	int x = inblock / (unit * unit);
        	int y = inblock % (unit * unit);
        	...
        	int blockNum2 = getBlockEntry(blockNum1, entrySize, x);
        	fresh = blockNum2 == 0;
        	if (fresh) {
        		if(mode == MODE.r)
                    return DirectBlock.hole;
        		// allocate a free block to the new block
        		if ((blockNum2 = createDiskBlock(blockNum1, entrySize, x)) == 0) {
        			// disk is full
        			return null;
        		}
        	}
        	
        	// now check the third indirect table
        	int blockNum3 = getBlockEntry(blockNum2, entrySize, y);
        	fresh = blockNum3 == 0;
        	if (fresh) {
        		if(mode == MODE.r)
                    return DirectBlock.hole;
        		// allocate a free block to the new block
        		if ((blockNum3 = createDiskBlock(blockNum2, entrySize, y)) == 0) {
        			// disk is full
        			return null;
        		}
        	}
        	...
        	// return the direct data block
        	return new DirectBlock(disk, blockNum3, blockOff, fresh);
        }
        
        // the file is extremely large,
        // which cannot put into the triple indirect block!!
        return null;
    }
    
    // a helper function to create a indirect block
    // and initialize all entries to 0 (empty data block number).
    // return 0 if disk is full.
    private int createDiskBlock() {
    	int blockNum = freeMap.find();
    	if (blockNum != 0) {
    		disk.write(blockNum, new byte[Disk.BLOCK_SIZE]);
    	}
		return blockNum;
    }
    
    // a helper function to create a data block
    // and insert its index into the indirect block at the offset entry.
    private int createDiskBlock(int indirectBlock, int entrySize, int offset) {
    	int blockNum = createDiskBlock();
    	if (blockNum == 0) {
    		return 0;
    	}
    	// write into the indirect block
    	byte[] buffer = new byte[Disk.BLOCK_SIZE];
    	disk.read(indirectBlock, buffer);
    	// move to the offset
    	offset = offset * entrySize;
    	for (int i = 0; i < entrySize; i++) {
    		buffer[offset + i] = (byte)((blockNum >> (8 * i)) & 0xFF);
    	}
    	// write it back to the block.
    	disk.write(indirectBlock, buffer);
    	return blockNum;
    }
    
    private int getBlockEntry(int indirectBlock, int entrySize, int offset) {
    	byte[] buffer = new byte[Disk.BLOCK_SIZE];
    	disk.read(indirectBlock, buffer);
    	// move to the offset
    	offset = offset * entrySize;
    	int blockNum = 0;
    	for (int i = 0; i < entrySize; i++) {
    		blockNum = (blockNum << 8) | ((int)buffer[offset + entrySize - 1 - i] & 0xFF);
    	}
    	return blockNum;
    }
